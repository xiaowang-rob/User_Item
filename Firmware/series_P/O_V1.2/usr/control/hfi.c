/**
 * @file    hfi.c
 * @brief   高频注入(HFI)无感位置估算
 * @note    基于αβ轴脉振方波注入 + PLL跟踪
 *          全程角度制 (degree)，纯电气角度
 */
#include "hfi.h"
#include "filter.h"
...
/**
 * @brief  HFI核心步进 (每控制周期调用)
 * @param  ialpha, ibeta  αβ电流 [A]
 * @param  u_alpha_h, u_beta_h  高频注入电压αβ分量 [V]
 *
...
/**
 * @brief  初始位置辨识 (磁饱和极性判断)
 * @param  id       d轴电流 [A]
 * @param  ualpha, ubeta  αβ轴输出电压 [V]
