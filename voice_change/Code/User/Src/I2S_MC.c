...
void HAL_I2S_RxCpltCallback(I2S_HandleTypeDef *hi2s)
{
    if (hi2s == &hi2s2)
    {
        ...
        {
            val24[i] = ((uint32_t)PCM_DMA[i * 4] << 8) + (uint32_t)(PCM_DMA[i * 4 + 1] >> 8);
            if (val24[i] & 0x800000)
            {
        ...
        if (USB_Audio_Port_Can_Update_Data() == true)
            USB_Audio_Port_Put_Data(val32, val32);
				HAL_I2S_Receive_DMA(&hi2s2,(uint16_t*)PCM_DMA, 128);
    }
}
