...
class ComPort(QObject):
    ...
    def _refresh_ports(self):
        ...
        selected_device = self._port_devices[current_index] if 0 <= current_index < len(self._port_devices) else None
        ...
        for port in ports_info:
            ...
            for pid, info in devices.items():
                pid_upper = pid.upper()
                patterns = [f"={vid}:{pid_upper}", f"VID:{vid}:PID:{pid_upper}", f"VID_{vid}&PID_{pid_upper}"]
                if any(p in hwid for p in patterns):
                    port_type = info['name']
                    break
                    
            # 2. 识别蓝牙
            if port_type == 'unknown' and any(k in hwid or k in desc for k in ['BTH', 'BLUETOOTH', '蓝牙', 'RFCOMM']):
                port_type = 'bluetooth'
            
            entry = {'device': device_name, 'type': port_type, 'display': f"{device_name} ({port_type})"}
            if device_name in self._current_ports and device_name not in [e['device'] for e in new_entries]:
    ...
    def connect(self):
        ...
        try:
            ...
            if port_type == 'unknown':
                self.ui_message.emit(MSG_TYPE_ERROR, "未识别设备：仅支持连接配置列表中的有线设备", True, 3000)
                self._update_ui_state()
                return False
            ...
            self.serial_port = serial.Serial(
                port=self._port_devices[index], baudrate=115200, bytesize=serial.EIGHTBITS,
                parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE,
                timeout=0.1, write_timeout=0.5, xonxoff=False, rtscts=False, dsrdtr=False
            )
            ...
            if self._is_bootloader_mode:
                ...
                self.ui_message.emit(MSG_TYPE_SUCCESS, "连接 bootloader，进入 IAP 模式", True, 2000)
            else:
                self._send_with_retry(Cidx.UC_CONNECT, bytes(), retries=2)
            ...
            if any(kw in error_msg for kw in ['PermissionError', '拒绝访问', 'Access is denied']):
                self._auto_connect_enabled = False
                p_name = self._port_devices[self.comport_list.currentIndex()] if 0 <= self.comport_list.currentIndex() < len(self._port_devices) else "Unknown"
    ...
    def confirm_device_handshake(self):
        ...
        self._pending_handshake = False
        self._update_ui_state()
        self.ui_message.emit(MSG_TYPE_SUCCESS, f"设备 {self.comport_list.currentText()} 握手成功，已连接", True, 1500)
        return True
    ...
    def _on_handshake_timeout(self):
        if self._pending_handshake:
            ...
            self.ui_message.emit(MSG_TYPE_ERROR, "设备握手超时（5s内未收到响应），请重试", True, 3000)

    def _build_packet(self, cmd_id, data_bytes):
        data_bytes = bytes(data_bytes) if isinstance(data_bytes, (bytes, bytearray, list)) else bytes(0)
    ...
    def _send_with_retry(self, cmd_id, data_bytes, retries=2):
        for i in range(retries + 1):
            if self._send_nowait(cmd_id, data_bytes): return True
            if i < retries: time.sleep(0.1 * (i + 1))
        return False
    ...
    def _receive_loop(self):
        ...
        while not self._stop_recv.is_set() and self.is_connected:
            try:
                ...
                if not self._stop_recv.is_set(): print(f"接收线程异常：{e}")
                break
        buffer.clear()
